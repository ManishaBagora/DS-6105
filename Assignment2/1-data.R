...
## [your code here]
df <- college[270:299,]
y=select(df,X,Grad.Rate)
z=arrange(y,desc(Grad.Rate))
filter(z,Grad.Rate >= 80)
...
## [your code here]
abc <- group_by(df,Private)
abc <- summarise(abc,max.tution=max(Outstate),min.tution=min(Outstate))
abc
abc1<-group_by(college,Private)
abc1 <- summarise(abc1,max.tution=max(Outstate),min.tution=min(Outstate))
abc1
